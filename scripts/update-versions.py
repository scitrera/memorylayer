#!/usr/bin/env python3
"""
Synchronize version numbers across all OSS subprojects.

Reads oss/versions.yaml as the single source of truth and updates:
...
Usage:
    python oss/scripts/update-versions.py           # from repo root
    python oss/scripts/update-versions.py --check   # dry-run: exit 1 if out of sync
    python oss/scripts/update-versions.py --verbose # show every file touched
"""
...
SCRIPT_DIR = Path(__file__).resolve().parent  # oss/scripts/
OSS_ROOT = SCRIPT_DIR.parent  # oss/
...
# (strategy, path relative to OSS_ROOT, *optional extra args)
# For 'marketplace', extra[0] = the project directory substring to match in
# the plugin's "source" field.
...
PROJECT_RULES: dict[str, list[UpdateRule]] = {
    ...
    "memorylayer-core-python": [
        ("pyproject", Path("memorylayer-core-python/pyproject.toml")),
        ("init_py", Path("memorylayer-core-python/src/memorylayer_server/__init__.py")),
    ],
    "memorylayer-mcp-typescript": [
        ("package", Path("memorylayer-mcp-typescript/package.json")),
    ],
    "memorylayer-sdk-langchain-python": [
        ("pyproject", Path("memorylayer-sdk-langchain-python/pyproject.toml")),
        ("init_py", Path("memorylayer-sdk-langchain-python/src/memorylayer_langchain/__init__.py")),
    ],
    "memorylayer-sdk-llamaindex-python": [
        ("pyproject", Path("memorylayer-sdk-llamaindex-python/pyproject.toml")),
        ("init_py", Path("memorylayer-sdk-llamaindex-python/src/memorylayer_llamaindex/__init__.py")),
    ],
    "memorylayer-sdk-python": [
        ("pyproject", Path("memorylayer-sdk-python/pyproject.toml")),
        ("init_py", Path("memorylayer-sdk-python/src/memorylayer/__init__.py")),
    ],
...
def load_versions() -> dict[str, str]:
    """Load and validate oss/versions.yaml."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Synchronize subproject versions from oss/versions.yaml",
    )
    parser.add_argument(
        "--check",
        action="store_true",
