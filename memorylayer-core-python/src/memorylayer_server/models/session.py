...
class Session(BaseModel):
    ...
    # Metadata
    metadata: dict[str, Any] = Field(default_factory=dict, description="Session metadata (client info, etc.)")

    # v2 additions for session lifecycle
    auto_commit: bool = Field(True, description="Auto-commit working memory on session end")
    committed_at: datetime | None = Field(None, description="When session was committed to long-term memory")
    ...
    @classmethod
    def create_with_ttl(cls, session_id: str, workspace_id: str, ttl_seconds: int = 3600, **kwargs) -> "Session":
        """Create a session with TTL in seconds."""
        return cls(id=session_id, workspace_id=workspace_id, expires_at=datetime.now(UTC) + timedelta(seconds=ttl_seconds), **kwargs)
...
class WorkingMemory(BaseModel):
    ...
    # Lifecycle
    ttl_seconds: int | None = Field(None, description="Optional TTL override (inherits session TTL if None)")
    created_at: datetime = Field(default_factory=lambda: datetime.now(UTC), description="Creation timestamp")
...
class SessionBriefing(BaseModel):
    ...
    workspace_summary: dict[str, Any] = Field(..., description="Workspace-level summary (total memories, recent activity, etc.)")
    recent_activity: list[dict[str, Any]] = Field(default_factory=list, description="Recent sessions/activity summaries")
    open_threads: list[dict[str, Any]] = Field(default_factory=list, description="Ongoing topics/threads")
    contradictions_detected: list[dict[str, Any]] = Field(default_factory=list, description="Memories with contradictory relationships")
    memories: list[dict[str, Any]] = Field(default_factory=list, description="Relevant memories for this session (v2 addition)")
