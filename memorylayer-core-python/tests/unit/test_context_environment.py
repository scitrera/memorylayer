...
class TestConfigConstants:
    ...
    def test_limits_config(self):
        ...
        assert DEFAULT_MEMORYLAYER_CONTEXT_QUERY_MAX_TOKENS == 8192
...
class TestRestrictedExecutor:
    ...
    async def test_list_comprehension(self, restricted_executor):
        ...
        result = await restricted_executor.execute("evens = [x for x in data if x % 2 == 0]", state)
        assert result.error is None
        assert state["evens"] == [2, 4]

    async def test_dict_comprehension(self, restricted_executor):
        state = {"keys": ["a", "b", "c"]}
        result = await restricted_executor.execute("mapping = {k: i for i, k in enumerate(keys)}", state)
        assert result.error is None
    ...
    async def test_for_loop_blocked(self, restricted_executor):
        state = {}
        result = await restricted_executor.execute("total = 0\nfor i in range(5):\n    total += i", state)
        assert result.error is not None

    async def test_while_loop_blocked(self, restricted_executor):
        state = {}
        result = await restricted_executor.execute("x = 0\nwhile x < 5:\n    x += 1", state)
        assert result.error is not None
    ...
    async def test_output_truncation(self, restricted_executor):
        state = {}
        result = await restricted_executor.execute('print("x" * 200)', state, max_output_chars=50)
        assert result.error is None
...
class TestSmolagentsExecutor:
    ...
    async def test_builtin_sum(self, smolagents_executor):
        state = {}
        result = await smolagents_executor.execute("data = [1, 2, 3]\ntotal = sum(data)", state)
        assert result.error is None
    ...
    async def test_comprehension(self, smolagents_executor):
        ...
        result = await smolagents_executor.execute("doubled = [x * 2 for x in nums]", state)
        assert result.error is None
...
class TestDefaultContextEnvironmentService:
    ...
    async def test_inject_json(self, service):
        result = await service.inject("s1", "config", '{"key": "value"}', parse_json=True)
        assert result["type"] == "dict"

    async def test_inject_invalid_json(self, service):
        result = await service.inject("s1", "bad", "not json{", parse_json=True)
        assert "error" in result
    ...
    async def test_hard_cap_enforcement(self):
        ...
        svc = DefaultContextEnvironmentService(v=v, executor=RestrictedExecutor())
...
class TestSchemas:
    ...
    def test_inspect_response(self):
        ...
        resp = ContextInspectResponse(variable="x", type="int", preview="42", size_bytes=28)
