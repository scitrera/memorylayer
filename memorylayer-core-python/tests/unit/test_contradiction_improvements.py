...
class TestEntityValueExtraction:
    ...
    def test_extracts_simple_is_pattern(self):
        triples = DefaultContradictionService._extract_entity_values("The server is Python")
        assert len(triples) >= 1
        subjects = [t[0] for t in triples]
        assert any("server" in s for s in subjects)

    def test_extracts_uses_pattern(self):
        triples = DefaultContradictionService._extract_entity_values("The project uses PostgreSQL")
        assert len(triples) >= 1
        values = [t[2] for t in triples]
        assert any("postgresql" in v for v in values)

    def test_extracts_runs_pattern(self):
        triples = DefaultContradictionService._extract_entity_values("The service runs Python 3")
        assert len(triples) >= 1

    def test_case_insensitive_extraction(self):
        triples = DefaultContradictionService._extract_entity_values("The API IS FastAPI")
        # Case-insensitive lowercased
        assert all(t[0] == t[0].lower() for t in triples)
        assert all(t[2] == t[2].lower() for t in triples)

    def test_returns_empty_for_no_match(self):
        triples = DefaultContradictionService._extract_entity_values("Hello world this has no entity patterns at all here")
...
class TestCheckSemanticConflict:
    ...
    def _make_memory(self, mem_id: str, content: str, embedding: list[float], workspace_id: str = "ws1", created_at: datetime = None):
        mem = MagicMock()
        mem.id = mem_id
        mem.content = content
        mem.embedding = embedding
        mem.workspace_id = workspace_id
        mem.created_at = created_at or datetime.now(UTC)
        return mem
...
class TestScanWorkspace:
    ...
    def _make_memory(self, mem_id: str, content: str, embedding: list[float], workspace_id: str = "ws1"):
    ...
    async def test_scan_finds_negation_contradiction(self):
        ...
        storage.get_recent_memories = AsyncMock(
            side_effect=[
                [{"id": "mem_a"}],
                [],
            ]
        ...
        storage.get_memory = AsyncMock(
            side_effect=lambda ws, mid, track_access=True: {
                "mem_a": mem_a,
                "mem_b": mem_b,
            }.get(mid)
        )
...
class TestWorkspaceContradictionScanHandler:
    ...
    def _make_handler(self):
        from memorylayer_server.tasks.workspace_contradiction_scan_handler import WorkspaceContradictionScanHandler
    ...
    async def test_handle_single_workspace(self):
        ...
        contradiction_service.scan_workspace = AsyncMock(
            return_value=[ContradictionRecord(workspace_id="ws1", memory_a_id="a", memory_b_id="b")]
        )
        ...
        def get_ext(ext_name, variables):
            from memorylayer_server.services._constants import EXT_CONTRADICTION_SERVICE
...
class TestConsolidationTaskHandler:
    ...
    async def test_handle_consolidates_cluster(self):
        ...
        storage.get_memory = AsyncMock(side_effect=lambda ws_id, mid, track_access=True: mem_map.get(mid))
