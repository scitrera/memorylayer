...
class TestNegationPatternDetection:
    ...
    def test_detects_use_vs_dont_use(self):
        assert DefaultContradictionService._has_negation_pattern("Use React for the frontend", "Don't use React for the frontend")

    def test_detects_enable_vs_disable(self):
        assert DefaultContradictionService._has_negation_pattern("Enable dark mode by default", "Disable dark mode by default")

    def test_detects_always_vs_never(self):
        assert DefaultContradictionService._has_negation_pattern("Always use type hints", "Never use type hints")

    def test_detects_true_vs_false(self):
        assert DefaultContradictionService._has_negation_pattern("Set debug to true", "Set debug to false")

    def test_detects_should_vs_should_not(self):
        assert DefaultContradictionService._has_negation_pattern("You should use async", "You should not use async")

    def test_detects_prefer_vs_avoid(self):
        assert DefaultContradictionService._has_negation_pattern(
            "Prefer composition over inheritance", "Avoid composition, use inheritance"
        )

    def test_detects_include_vs_exclude(self):
        assert DefaultContradictionService._has_negation_pattern("Include logging in all services", "Exclude logging from services")

    def test_detects_bidirectional(self):
        """Negation should be detected regardless of which text has the positive term."""
        assert DefaultContradictionService._has_negation_pattern("Don't use tabs", "Use tabs for indentation")

    def test_case_insensitive(self):
        assert DefaultContradictionService._has_negation_pattern("ALWAYS run tests", "Never run tests")

    def test_no_negation_for_unrelated_texts(self):
        assert not DefaultContradictionService._has_negation_pattern("The sky is blue", "Python is a programming language")

    def test_no_negation_for_agreeing_texts(self):
        assert not DefaultContradictionService._has_negation_pattern("Use Python for backend", "Use Python for data science")
...
class TestContradictionDetection:
    ...
    async def test_check_new_memory_finds_negation(self, storage_backend, embedding_service, workspace_id):
        ...
        found = any(c.memory_a_id == mem2.id and c.memory_b_id == mem1.id for c in contradictions)
        assert found, f"Expected contradiction between {mem2.id} and {mem1.id}"

    async def test_check_new_memory_no_contradiction_for_unrelated(self, storage_backend, embedding_service, workspace_id):
    ...
    async def test_check_new_memory_skips_without_embedding(self, storage_backend, workspace_id):
    ...
    async def test_check_new_memory_nonexistent_returns_empty(self, storage_backend, workspace_id):
...
class TestContradictionResolution:
    ...
    async def test_resolve_keep_a_soft_deletes_b(self, storage_backend, embedding_service, workspace_id):
        ...
        resolved = await service.resolve(workspace_id, contradiction.id, "keep_a")
    ...
    async def test_resolve_keep_both(self, storage_backend, workspace_id):
    ...
    async def test_resolve_nonexistent_returns_none(self, storage_backend, workspace_id):
...
class TestGetUnresolved:
    ...
    async def test_get_unresolved_returns_pending(self, storage_backend, workspace_id):
    ...
    async def test_get_unresolved_excludes_resolved(self, storage_backend, workspace_id):
    ...
    async def test_get_unresolved_respects_limit(self, storage_backend, workspace_id):
